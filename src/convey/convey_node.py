...
class ConveyorNode(Node):
    def __init__(self):
        ...
        # ROS 2 Publisher 설정 (상태 발행 및 알림)
        self.status_publisher = self.create_publisher(String, 'conveyor_status', 10)
        self.alert_publisher = self.create_publisher(String, 'conveyor_alert', 10)

        # ROS 2 Service 설정 (메인 노드에서 명령 수신)
        self.command_service = self.create_service(
            SetBool, 'conveyor_command', self.handle_command)

        # 상태 변수
        self.current_status = "Idle"  # 현재 상태
        self.previous_status = None  # 이전 상태 (상태 변화 감지용)

        # 타이머 설정
        self.timer = self.create_timer(0.1, self.publish_status)  # 상태 발행 주기
        self.usb_check_timer = self.create_timer(1.0, self.check_usb_connection)  # USB 연결 상태 확인 주기
    ...
    def handle_command(self, request, response):
        ...
        try:
            ...
            command = "Start" if request.data else "Stop"
            self.serial_port.write(f"{command}\n".encode())  # 명령 전송
            self.get_logger().info(f"Received command: {command}")

            # 상태 업데이트
            self.current_status = "Running" if request.data else "Stopped"
    ...
    def send_alert(self, emergency=False):
        ...
        try:
            ...
            self.get_logger().error(f"Error sending alert: {e}")
